 ...
 * 方案與價目。
 *
 * 兩個**生效語意**必須讓營運看見（否則他們會以為改了就影響當期）：
 * 1. 調價是新增一列價格史（後端 `UpsertPlanPriceTx`），現行價取 `effective_from` 最新者；
 *    既有期別的金額在開立時已快照 → 調價只影響之後新開的期別，不回溯改帳。
 * 2. 方案權益是「下一期的判定來源」：改權益影響之後開的期別與該方案的租戶判定；
 *    已開的期別與**租戶層級的例外**（override）優先於方案。
 *
    ...
    >
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            調價是新增一列價格史，從現在起成為該計費週期的現行價；已開立的期別金額已有快照，
            不受影響（不會回溯改帳）。
          </DialogDescription>
        </DialogHeader>
...
function EntitlementDialog(props: {
  ...
  const matrix = createQuery(() => ({
    ...
    enabled: props.planCode !== undefined,
  }));
    ...
    >
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            權益是下一期起的判定來源，也會改變用此方案租戶的判定（租戶層級的例外優先於方案）。
        ...
        {queryBoundary(matrix, (m) => (
          <WriteForm
            ...
            validate={() => {
              ...
              if (m.features.every((f) => f.code !== featureCode())) {
                return "這個功能不在後端的功能定義內（未定義的功能一律拒絕），請從清單中選。";
              }
        ...
        data.plans.length === 0 ? (
          ...
          <div class="space-y-4">
            <p class="text-sm text-muted-foreground">
              調價與權益變更都是下一期生效：調價新增一列價格史，已開立的期別金額已有快照，
              不會被回溯改帳；權益只影響之後開的期別與該方案租戶的判定（租戶例外優先）。
            </p>
