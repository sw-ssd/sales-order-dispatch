...
describe("ensureSession（session 探針）", () => {
  ...
  it("連線失敗（非 401）一樣視為未登入：寧可擋在登入頁", async () => {
    getOperatorSelf.mockRejectedValue(new ConnectError("後端不可用", Code.Unavailable));
    await expect(ensureSession()).resolves.toBe(false);
    expect(sessionStatus()).toBe("anonymous");
  });
  ...
  it("單一探針：並行與後續呼叫合計只打後端一次", async () => {
    ...
    await ensureSession();
    expect(getOperatorSelf).toHaveBeenCalledTimes(1);
  });
  ...
  it("成功結果不得永久快取：過了 TTL 會重新探針", async () => {
    ...
    try {
      ...
      await ensureSession();
      expect(getOperatorSelf).toHaveBeenCalledTimes(2);
    } finally {
      now.mockRestore();
    }
  });
  ...
  it("舊探針晚回不得覆寫新探針的狀態（TTL 替換競態）", async () => {
    ...
    const first = Promise.withResolvers<{ operatorId: string; email: string; role: string }>();
    const second = Promise.withResolvers<{ operatorId: string; email: string; role: string }>();
    const now = vi.spyOn(Date, "now");
    try {
      now.mockReturnValue(0);
      getOperatorSelf.mockReturnValueOnce(first.promise);
      const p1 = ensureSession();
      now.mockReturnValue(PROBE_TTL_MS + 1);
      getOperatorSelf.mockReturnValueOnce(second.promise);
      ...
      expect(sessionStatus()).toBe("anonymous");
      first.resolve({ operatorId: "42", email: "ops@example.com", role: "admin" });
...
describe("logout", () => {
  ...
  it("登出時已有飛行中的探針：其成功回來不得把狀態翻回 authenticated", async () => {
    ...
    const { promise, resolve } = Promise.withResolvers<{ operatorId: string; email: string; role: string }>();
    getOperatorSelf.mockReturnValue(promise);
    const inFlight = ensureSession();
    logout();
    resolve({ operatorId: "42", email: "ops@example.com", role: "admin" });
