...
| G4 | **輸入金額從未與期別快照比對** | `AmountCents` 只出現在事件／稽核 payload | 營運輸入 3000 而期別 1950 → 帳面 1950、**輸入被默默丟棄** | ② 明定：`0` → 採快照；非 0 且不符 → `FailedPrecondition`＋碼 `PLAT-3002`（差異以 `note` 記錄） | ✅ 已修 |
...
| G8 | **部分付款／溢收短收無處記錄** | `RecordPayment` 只支援「整期 paid」或「no-op」 | v1 明示不支援部分付款；`subscription_periods.note` 記短收/溢收；金額不符（非 0 且 ≠ 快照）→ `FailedPrecondition`＋碼 `PLAT-3002` | ✅ 已定案並修入 spec 與 ③ |
...
```mermaid
flowchart LR
    P0["P0 安全底線 ✅（2026-09-20）<br/>Plan A（RLS）＋ Plan D（錯誤碼）＋ G1-G5 修計畫"] --> P1["P1 能收錢<br/>Plan B 全部 ＋ Plan C T1-T7"]
...
| P0-5 | **Plan D 全 7 任務（錯誤碼骨架）**：`ErrorInfo` proto、`internal/errcode` registry、`trace_id`、`toConnectError` 走 registry、首批碼落地、基線守門＋碼表產生、文件對齊（T5b 延後） | ✅ **已完成（2026-09-20）**。T1–T3 可與 P0-1 並行（proto／registry／obs 檔案不重疊）；**T4–T5 的解鎖原因是 P0-1（Plan A）已完成**：兩者都改同一批檔案（`company_service.go`：A 改交易取得、D 改 `toConnectError`／`requireScope`；`auth_handler.go`：A 加系統範圍包裝、D 改錯誤碼）→ 同檔並行的 rebase 成本高於序列化等待。T6–T7 不受限。現況：**21 碼**（SYS 7／AUTH 7／PLAT 4／CUST 3）、**14 碼已落點**、基線 **98 行／233 呼叫點**、碼表與三端常數由 `go generate ./internal/errcode` 產生且 CI 驗同步。**T5b（`PLAT-*` 落點）待 Plan B 落地後執行** |

**為什麼錯誤碼不能等 P1 之後**：它是**對外契約**，P1 一旦出貨，補碼就是 breaking change；且 P1 的配額錯誤（`PLAT-5001/5002`）必須在 Plan B 開工前可用，否則 B／C 的錯誤建構要重寫一次。（**已達成**：`internal/errcode` 已可用、四個 `PLAT-*` 碼已註冊；Plan B／C 直接引用，落點與斷言由 Plan D Task 5b 收尾。）
...
**錯誤碼**：配額與收款錯誤**已使用 `PLAT-*` 碼**（Plan D 已完成骨架）：`PLAT-5001` `PlatformLimitExceeded`／`PLAT-5002` `PlatformFeatureNotInPlan`／`PLAT-3001` `PlatformSubscriptionInactive`／`PLAT-3002` `PlatformPaymentConflict`；Plan B／C 直接引用 `errcode.*`，不得自建裸 connect 碼（碼表：`docs/error-codes.md`）。
...
08 派車看板 → 10 logistics（D32）→ 07 通知（催收提醒與下單推播的依賴，G6/G7 之後才會有自動提醒需求）→ App 業務頁與 G12 的錯誤呈現 → G10（稽核保留期）／G11（資料匯出）／G13（覆蓋率門檻）。

---

## 5. 一頁掌握：接下來三步

**P0 已完成（2026-09-20）**：G1–G5 已修、G6／G7 已定調並寫回 ②／③ 與 spec；G15 已寫入 `backend/AGENTS.md` §3；**Plan A（RLS）10 tasks ＋ Task 11 收尾**全數落地；**Plan D（錯誤碼 T1–T7）**完成（§4 的 P0-5，含 21 碼、基線守門、三端常數與碼表產生＋CI 同步）。

1. **開工 P1-1：Plan B T1–T6**（platform schema、store、entitlement、計數器、守衛）——權益判定是所有寫入路徑的守衛介面，先定；`internal/errcode` 已可用，`PLAT-*` 直接引用。
2. **接 P1-2／P1-3：Plan B T7–T12 → Plan C T1–T7**（`platform/v1`／operator 認證／唯讀 RPC／投影／seeds；狀態入口／money／帳務 store／`RecordPayment`／生命週期／consumer／cron）。
3. **達 P1 交付判準**（開通 → 記收款 → 逾期 `past_due` → 寬限後 `suspended` → 補款復原，且排程與告警自動化）；並在 Plan B 落地後執行 **Plan D Task 5b**（`PLAT-*` 四碼的落點與測試斷言）。
