...
| 8 | 跨期衝突的**錯誤形狀** fake 與真 store 不同；`cancelled_at` 無法由 fake 表達 | **部分已修**（三層行為已釘住：fake 層跨期拒絕、真 SQL 層唯一索引拒絕、billing 層 `PLAT-3002`＋不寫事件／稽核，見 `fake_billing.go:329` 註解。殘：`cancelled_at` 只由整合測試覆蓋） | 讓 fake 回 `*pgconn.PgError` 等價物，或把取消時間語意限定在整合測試 | `platform/store` 的 fake（低） |
