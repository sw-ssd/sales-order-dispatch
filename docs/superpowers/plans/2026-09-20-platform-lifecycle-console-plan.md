...
| 8 | 跨期衝突的**錯誤形狀** fake 與真 store 不同；`cancelled_at` 無法由 fake 表達 | **已修**（三層行為已釘住；`store.Subscription.CancelledAt`＋fake 建模真 store 的 CASE 語意，`TestFakeBillingCancelledAtNotAdvancedOnRepeat` 為證） | 讓 fake 回 `*pgconn.PgError` 等價物，或把取消時間語意限定在整合測試 | `platform/store` 的 fake（低） |
| 9 | `PLAT-3002` 曾把**基礎設施錯誤**也講成收款衝突 | **部分已修**（唯一鍵跨期衝突現走 `kind=cross_period`，見 `billing.go:215`；基礎設施錯誤走 `SYS-9000` 分流，見 `SYS-9000` 的 toConnectError 預設分支。殘：spec §4.3「不得據此放棄重試」的重試語意仍待結構化鍵） | 在 `ErrorInfo.details` 加專屬鍵（例 `details.conflict`）；console 已有「無法分辨→通用指引」 | `platform/v1` 下一個計畫 |
| 10 | `RecordPayment` 的 no-op 分支曾**不比對金額** | **已修**（`3c1fe60`：同交易號金額不符回 `PLAT-3002 kind=amount_mismatch`，現 `billing.go:171` 為證） | 在 no-op 分支比對金額並回 `PLAT-3002` | `billing` 後續維護（低） |
| 11 | 負數金額 → `SYS-1001` 是**未列於文件**的 Extra；`ActorOperatorID` 未驗證；`now` 不可注入；`allowedTransitions` 其餘列待補測 | **部分已修**（`1b3b9d5`：五個帳務入口驗 `ActorOperatorID`，0 即 `SYS-1001`；`Billing.now` 欄位可注入，現 `billing.go:53` 為證。殘：Extra 文件、`allowedTransitions` 其餘列） | 補文件／補驗證／補測試；webhook 落地時優先驗 actor | `billing` 後續維護（低～中） |
| 12 | `PLAT-3001` 的訊息對「資料／價目設定錯誤」語意錯配；「服務中」清單在三處各自表述；`MarkPastDue`／`SuspendOverdue` 的狀態更新曾**無列鎖／CAS** | **部分已修**（`ca2f587`：`SetSubscriptionStatusTx` 加 `expectedStatus` CAS，`lifecycle.go` 三處跳過。殘：價目錯誤訊息語意） | 改用 `UPDATE … WHERE status = <預期>` 的 CAS | `billing` 後續維護（中） |
| 13 | 期別錨點的 fallback 曾**靜默** | **已修**（`c963b82`：缺第一期時 `log.Printf` 留痕＋行為測試，現 `lifecycle.go:85` 為證） | 在排程加資料完整性檢查並留痕（log／事件） | 排程告警（中） |
| 14 | 新語意殘角：當期已 `paid` 但下一期開不出來（價目缺失／`billing_cycle` 非法 →`PLAT-3001`）→ 訂閱**永遠停在 active、既不被催收也不再被開帳** | **已修**（`35c0d8e`：排程摘要 `Unbilled` 計數，現 `run.go:177` 為證） | 加告警「服務中卻沒有 open 期別」（指標或排程摘要欄位） | 排程告警（中） |
| 15 | `DispatchOnce` 回傳值語意曾易誤讀 | **已修**（`ca400c5`：區域變數改名 `claimedN`＋註明「認領數非成功數」，現 `consumer.go` 為證） | 重新命名或另回一個欄位 | `consumer` 後續維護（低） |
| 16 | actor 解析失敗不再中止整趟 → 每個 mapped 事件各 append 一條同根因錯誤 → `errors.Join` 訊息 O(N) 膨脹 | **已修**（`9fc6cb1`：`actorFailed` 旗標同趟只記一次，現 `consumer.go` 為證） | actor 失敗記一次並跳過其餘 | `consumer` 後續維護（低） |
...
| 18 | `Params.EventBatch` 與 `const EventBatch` 兩層入口；`--date` 可任意倒回（會把 `grace_until` 設成過去）；排程待收款曾含 G5 | **部分已修**（`42b4b27`：`--date` 不得早於現在 24h；`79f8350`：`OverdueReceivablePeriods` 加 `identifier <> 'platform'`。殘：`EventBatch` 雙入口、`Summary.Locked` 欄位序） | 排程端套同一排除條件；`--date` 限制不得早於上次執行 | `platform/cron` 後續維護（低～中） |
| 19 | 整合測試的 `GRANT EXECUTE ON pg_advisory_unlock` 還原排在 `t.Fatalf` 之後；`run.go:331` 註解措辭；`run_test.go:356` 註解筆誤（43/44）；`fakeLocker` 的 `ctx.Err()` 閘門與真實 locker 行為分歧；**永久壞租戶讓每趟 exit 1** | **部分已修**（還原已走 `t.Cleanup`，現 `run_integration_test.go:171` 為證；`fakeLocker` 已註明 `WithoutCancel` 前提，現 `run_test.go:192` 為證。殘：註解措辭／筆誤、永久壞租戶 exit 1） | 還原改 `t.Cleanup`；日後考慮「降級成功」模式以免告警噪音 | `platform/cron` 測試與告警（低） |
| 20 | 權益快取其餘 P3：`InvalidateAll` 首個 `DEL` 失敗即 return；快取錯誤的「不支援全量失效」曾用 `errors.New`（T9 已提供 sentinel）；`openEntitlementCache` 的 `MemoryCache` 分支今日不可達；快取故障時 **log 無節流**；`TestJudgementSurvivesCacheFailure` 改動**行程級 logger**；`cmd/platform-cron` 未關 Valkey client | **部分已修**（`ae99974`：`InvalidateAll` 改 `errors.Join` 收斂；`38f63bc`：故障 log 節流，現 `service.go:97` 為證。殘：sentinel 歷史、`MemoryCache` 分支、行程級 logger 測試、未關 client） | `errors.Join` 收斂部分失敗；log 節流；測試改注入 logger | `entitlements` 快取（低） |
| 21 | 註解宣稱「正確性**不依賴 TTL**」略強於實情（cache-aside 競態：提交前 miss、提交後 `Set` 可把舊快照蓋過剛做的 `Delete`） | **spec §4.4 已改寫為「失效由寫入方驅動；TTL 是最長收斂上界」並標日期**；**程式註解（`entitlements/valkey.go:20`／`cache.go:10`）未動**（本任務範圍不含程式碼） | **已修**（`6a28cdd`：`valkey.go`／`cache.go` 註解已同步 spec §4.4 措辭） | `entitlements` 後續維護（文件） |
| 22 | `SetSeatCount` 守衛的交易外讀取（TOCTOU）；`UpdateBillingSettings` 的 before 交易外讀取等六項 | **部分已修**（`89cfbc7`：`ChangePlan` 存在檢查優先於方案檢查；`04b7a2c`：TOCTOU 註記收斂方向為 billing 內复核，現 `platform_admin_service.go:562` 為證；`40bc827`：doc comment 歸位。殘：讀取搬進交易、before 快照、`ttl<=0` 單元守門） | 把讀取搬進交易；補單元守門；統一錯誤碼優先序 | `platform_admin_service` 後續維護（低～中） |
| 23 | console 曾不依角色隱藏操作 | **已修**（`bbdf7d6`：新增 `GetOperatorSelf`＋探針取回 `operatorSelf`，現 `session.ts:22` 為證） | 新增 `GetOperatorSelf`（或等價 RPC）＋前端依角色 disable | `platform/v1` 下一個計畫 |
| 24 | T9 修正後**整合測試夾具殘留**（把 `ops-ghost@example.com` 留在 active admin）；advisory lock 的保證**依附 isolation level** | **部分已修**（`effa908`：ghost 夾具收尾改回 `disabled`，稽核 FK 不斷；isolation 已寫進 AGENTS.md §11-16。殘：在部署文件明示 `default_transaction_isolation`） | 清理 fixture；在部署文件明示 `default_transaction_isolation` | `platform` 測試（低）／部署文件 |
| 25 | `logout()` **不取消已在飛行中的探針**（其 `.then` 仍會 `setStatus("authenticated")`）；TTL 讓「飛行中探針被替換」成為新路徑；`/login` 精確字串對未來子路由會誤套外框；`session.test.ts` 的 mock 有第三份字面值 | **部分已修**（`44ffb92`：回來先查 `loggedOut`；`6c09062`：`probeGen` 世代計數，現 `session.ts:55,62` 為證。殘：`/login` 精確字串、第三份字面值） | 在 `.then` 內檢查 `loggedOut`／身分，並補反向斷言 | `platform-console` `lib/session`（低） |
| 26 | `TenantDetailPage.tsx:249` 的 `now` 只在元件建立時取一次（頁面開著時到期的例外仍算生效）；`:80` 到期日檢查寬鬆但訊息說「格式須為 RFC3339」 | **部分已修**（`1c651f1`：`now` 每分鐘 tick，現 `TenantDetailPage.tsx:504` 為證。殘：`:80` 寬鬆檢查訊息） | 用 signal 讓 `now` 隨時間前進；放寬或收緊訊息 | `platform-console`（低） |
| 27 | 租戶詳情曾缺試用到期／寬限期 | **已修**（`1d519fc`＋`d59f7cd`：proto 加 `trial_ends_at`／`grace_until` 三端重產＋卡片顯示，現 `platform.proto:19-20` 為證） | 在投影加欄位（動 proto） | `platform/v1` 下一個計畫 |
| 28 | **沒有「列出某訂閱的期別」RPC** → 租戶詳情無法顯示期別歷史 | **已修**（`44f8b8e`：新增 `ListSubscriptionPeriods` 三端重產，整合 `ok=27` 為證） | 新增 `ListSubscriptionPeriods`（或把期別塞進 `GetTenantResponse`） | `platform/v1` 下一個計畫 |
| 29 | 權益投影的 `ambiguous` 論述曾寫「會有第二筆」 | **已修**（`e19a764`：措辭修正＋恰一筆邊界斷言，現 `entitlements.ts:16` 為證） | — | `platform-console` `lib/entitlements.ts`（低） |
| 30 | `PLAT-3002` 的**字串比對**（依 `details.reason` 的中文關鍵詞分流兩種語意）；`MarkPeriodPaidTx` 唯一鍵衝突**無** `details.reason` → 走通用指引；CSV 下載未真機驗證；**收款表單未收 `invoice_status`／`buyer_tax_id`／`carrier`**（proto 有、後端可同筆寫入）；CSV 只含目前這一頁；稽核頁與收款頁自身的查詢錯誤態無專屬測試 | **部分已修**（`08a118e`：結構化 `kind` 鍵，console 優先走 kind；`cd8a338`：收款表單收開票三欄；CSV 只含本頁已在文案明示，現 `ReceivablesPage.tsx:236` 為證。殘：唯一鍵衝突無 reason、CSV 未真機驗證、錯誤態專屬測試） | 加結構化 `details` 鍵；開票三欄收進表單；補錯誤態測試 | `platform/v1` 下一個計畫／`platform-console` |
| 31 | 清空最後一頁後 `page` 停在 2 而 `lastPage` 變 1 → 曾顯示「**第 2 / 1 頁**」的矛盾頁碼 | **已修**（`b6d55cf`：顯示值夾住 `lastPage`，現 `pagination.tsx:20` 為證） | 當頁變空時把頁碼夾回 `lastPage`，或讓 Pagination 顯示值夾住 | `platform-console`（低） |
| 32 | `PlanBanner` 測試與投影手抄等四項 | **部分已修**（`8f6cfe9`：`queryKey` 共用契約測試，改 key 先紅。殘：`USABLE_STATUSES` 手抄副本、數值分支前綴、兩級色階） | 把 allow-list 寫成後端契約測試；色階分級 | `frontend` account 功能（低） |
| 33 | 無訂閱投影曾列 `enabled=false` 假列 | **已修**（`f429ee9`：`statusNone` 時用量列為空，`TestFakeBillingCancelledAtNotAdvancedOnRepeat` 同批為證；前端 `isUnprovisioned` 照常顯示未開通文案） | — | `platform/entitlements` 投影（訂閱指派落地時） |
| 34 | **store 層的 `reason` 只擋空字串，不 trim**（`recordAuditTx`：`postgres/billing.go` 與 `fake_billing.go` 各一份、`admin_writes.go` 的 `RecordAuditTx` 只是轉呼叫） | **已修**（`7083d9c`：`recordAuditTx` 與 fake 皆 `TrimSpace` 後判定，現 `billing.go:567` 為證） | — | `platform/store` 後續維護（低～中） |
...
| 36 | **開通時傳入不存在的 `company_id` → FK `23503` 被 `toConnectError` 收成 `SYS-9000`**（console 只能選既有租戶，故僅自製客戶端可達） | **已修**（`dabce6e`：先以 `GetTenant` 驗存在，不存在回 `SYS-4002`，現 `platform_admin_service.go:503` 為證） | 服務層先以租戶投影驗存在（回 `SYS-4002`），或把 23503 分流成 4xx | `platform/v1` 下一個計畫（低） |
| 37 | **價目 `base=seat=0` 仍能開出 0 元期別** | **已修**（`e10e977`：`base` 與 `seat` 同為 0 即拒，現 `platform_admin_service.go:890` 為證） | 價目寫入路徑擋 0，或為免費方案立旗標 | 產品語意定調（低） |
| 38 | **`subscription.created` 對 consumer 是未知型別**（記一行 log 後認領，無副作用） | **已註明歸屬**（`25065b3`：註明日後語意歸 `actions`，不得另起 consumer。行為不變） | 日後要在產品域對開通做事時，於 consumer 的 `actions` 明列語意 | `platform/consumer`（低） |
| 39 | **console 的開通按鈕顯示條件綁在「同一公司只能有一份未取消合約」**（只在 `none`／空／`cancelled` 顯示，與部分唯一索引同義） | **已收斂**（`8543809`：抽成 `canOfferSubscription`，後端放寬時同步放寬） | 後端若放寬成「同公司多份合約」，前端要同步放寬（否則介面比後端保守而不自知） | 後端放寬時同步（低） |
| 40 | **`trial_ends_at IS NULL` 的 `trialing` 訂閱無告警**：`ExpireTrials` 的謂詞刻意只認 `trial_ends_at IS NOT NULL`（不讓排程猜），而這種列**永遠停在試用**（可用、不催收、不凍結） | **已修**（`d0f13dd`：排程摘要 `StuckTrialing` 計數＋唯讀查詢，現 `run.go:185` 為證） | 加檢查（指標／排程摘要欄位）或 `CHECK (status <> 'trialing' OR trial_ends_at IS NOT NULL)` | 排程告警／migration（中） |
| 41 | **試用期長於一期時，第一期會在試用期間到期且不被催收**（第一期一律自 `now` 起算，而 `MarkPastDue` 只掃 `active`） | **已註明刻意語意**（`e04d654`：改前不得動謂詞；`lifecycle.go:150` 為證。產品語意未定調前不修） | 「試用期不開帳」需同時改 `EnsureNextPeriod` 與催收謂詞 | 產品語意定調（中） |
| 42 | **開通只有 RPC／console 一條路徑**：seed／CLI 若要種 demo 訂閱**必須**呼叫 `billing.CreateSubscription`，不得直接 INSERT（否則第一期、事件、稽核、價格快照全缺） | **慣例已文件化**（AGENTS.md §11 開通路徑說明，現 `AGENTS.md:191` 為證） | — | 慣例（已文件化） |
| 43 | **`store.BillingStore.MarkEventDispatchedTx` 只剩測試在用**（consumer 走 `claimSQL` 的條件式認領）；移除會連帶刪掉 fake／整合測試的覆蓋 | **已註明測試專用**（`1ff2eb7`：`fakeTx.Claim` 改條件式，介面 doc 標測試專用，現 `store.go:240` 為證） | 若 consumer 改走 store 的標記，兩者合併成一個 | `platform/store`（低） |
| 44 | **`money.YearlyFromMonthly` 沒有生產呼叫端**（proto 與 DB 都還沒有 `discount_bps`） | **維持保留**（函式註解＋AGENTS.md §11-18 已標明保留理由；`5b926f7` 另修其四捨五入溢位） | 價目寫入路徑加上折扣時由它負責算，勿另寫一份 | `platform/money`（低） |
