...
>
> **現況對齊：2026-09-25**（本輪：App 改密碼頁（A3 首登強改弱閉環）＋公告前台（後端前台過濾 RPC＋Web 首頁輪播/消息＋App 首頁 tab）＋FCM 推播環境控制）。①**改密碼頁**：App `ChangePasswordPage`（受限態/主動更改同一份表單）、`AuthRepository.changePassword` 走已認證 transport、登入依 `must_change_password` 優先於主/子分流導向；另修**兩個真 bug**——`Login` 完全沒檢查臨時密碼效期（規格明訂過期須拒絕登入）、App 無 `ErrorInfo` 解碼導致 AUTH-3002/3003/3004 全顯示成同一句。②**公告前台**：`ListActiveAnnouncements`（is_active＋上下架時間窗＋`deploy_web`/`deploy_app` 平台過濾；未帶 platform 一律 `invalid_argument`，不預設以免一邊看到不該投放的公告）、`announcement` 補 customer `read`（主帳號由 primary_account 推導自動排除）、五條公告 RPC 進 `protectedRPC`；Web `AnnouncementBoard`（Dashboard 掛載，真瀏覽器驗證）＋App 首頁 tab（`PageView` 輪播＋news/article 列表）。③**FCM 推播**：`config/fcm.go` 環境控制（`fake|fcm`；production 設 fake **拒絕啟動**）、`FCMSender`（HTTP v1／`JWTConfigFromJSON` 驗 service account 型別／失效 token 與暫時性失敗分開判定／無裝置標 `failed` 不假裝 sent）、App `PushRegistration` 掛 session 生命週期。④順修：sqlite 以字串比較時間導致跨時區時間窗失效（統一 UTC 比較）。驗證：backend `task check` EXIT=0、整合 EXIT=0、frontend 376 tests、app 40 單測＋**iOS 3:43 與 Android 3:43 五分頁實機全綠**、Web 真瀏覽器截圖。
>
...
| 計畫 | 路徑 | 範圍 | 狀態 | 說明 |
|------|------|------|------|------|
| 主計畫 | `2026-08-05-sales-order-1.0-subproject-implementation-plan.md` | 三子專案 50 Tasks / 5 Waves | 🟡 部分 | Wave 1 多數、Wave 2 部分已由 backend 01~04 分批落地（見下列分域計畫）；Wave 3~5 已大部分落地：訂單／退貨／通知／派車／列印後端＋Web 業務頁全數完成，App 業務畫面（訂單/退貨/通知/首頁公告）與改密碼頁已落地（2026-09-24~25，Task 47/48/49 全數完成）；**Task 50 部署與維運未開始**（無 Helm／k8s／metrics 端點，是唯一的上線阻斷項） |
...
| 02-tenancy-users | `backend/2026-08-17-backend-02-tenancy-users-plan.md` | 多租戶與使用者 | 🟡 部分 | Company/Department/Role CRUD 已實作；UserService 7 支 RPC 已落地（含稽核＋`dataScopeForUser`）；公司停用連鎖(2.1.3)（2026-09-18, A2）；公司/部門改**軟刪除**（partial unique index）＋刪除競態互斥鎖（FOR SHARE／FOR UPDATE）＋清單 `sort/desc` 白名單（2026-09-20）；**主帳號連鎖（D22）已落地（2026-09-25：停用客戶主帳號 → 同交易連鎖停用全部子帳號（status＋tv+1＋逐筆稽核帶 `cascade_from_primary`）；連鎖收斂到單一 helper，`Deactivate` 與 `UpdateUser(status→inactive)` 兩條路皆走它）；**Logo 上傳與消費面已落地（2026-09-22：`POST /companies/{id}/logo`（company_admin 限所屬公司）＋`GET /me`＋Web 上傳對話框/側邊欄顯示）**；**`X-Api-Token` 已落地（2026-09-25，尚無消費者：排程直連 DB）**；**店家自助帳號管理 Task 6.7 前後端皆落地（2026-09-25：後端 `CustomerAccountService`＋App 帳號管理頁與兩條深層連結）**；殘：App 改密碼頁（臨時密碼登入會收到 AUTH-3004）、深層連結網域與 `/.well-known/*` 未部署 |
...
| 07-notifications | `backend/2026-08-17-backend-07-notifications-plan.md` | 通知與 FCM | 🟡 部分 | 後端約 85%：四表 schema（00041）＋渲染＋通知中心＋DeviceService＋Sender／Failmark＋四路觸發（下單／專屬／退貨審核／派車）＋00042 RLS ENABLE＋FORCE；**Web 通知中心 `b193544`**（清單/未讀篩選/單列與整頁標記已讀＋`notification` 權限資源）；派車 adapter 已接 08（`notification_triggers.go` dispatch）；**App 通知中心已落地（2026-09-24：`9865b3c`）**；**FCM 推播已落地（2026-09-25）**：`FCM_PROVIDER=fake|fcm` 環境控制（production 設 fake 則拒絕啟動）、`FCMSender` 走 HTTP v1（service account 憑證型別驗證、失效 token 判定與回報、無裝置標 `failed` 不假裝 sent）、App `PushRegistration` 掛在 session 生命週期（登入註冊／登出與改密碼註銷）；殘：Firebase 原生設定檔未提供（缺席時靜默跳過，站內通知不受影響） |
...
| app-flutter-stack | `app/2026-08-04-app-flutter-stack.md` | App 技術棧與認證基礎（D29） | 🟡 部分 | Task 1（骨架）✅、Task 5/6/7（token/auth transport/router）部分；**根佈線與業務畫面已落地 2026-09-24**（CacheProvider+fquery、Bearer 401 單飛重試、訂單/退貨/通知三件套 Material/Cupertino 自適應 `9865b3c`＋實機整合測試）；core/config、Sembast 鏡像未落地；**2026-09-25 追加**：改密碼頁（A3 首登強改）、首頁公告（Task 49）、推播註冊 `PushRegistration`（缺 Firebase 原生設定檔時靜默跳過） |
| frontend-auth-users | `frontend/2026-09-18-frontend-auth-users-plan.md` | Web 中台 auth/users/ability | 🟡 部分 | 17/17 打勾：登入雙 tab、403、Google OIDC、公司/部門/角色/使用者四頁＋PermissionMatrix＋分頁、`requireAbility` 路由守衛已完成；另客戶主檔頁 `206192a`、訂單管理頁 `6953b02`、商品總表頁 `61feab2`、派車看板 `e3ed769`、車次主檔 `e950b23`、單據列印 `13c76f4`、退貨 `549933c`、通知中心 `b193544`、稽核 `834f575`、客戶地址簿/聯絡人 `183d205`、部門級主檔三頁 `0a590d0`、客戶專屬商品對話框 `6192c13`、公告管理頁 `943e3c1`，以及**公告前台 `AnnouncementBoard`（2026-09-25，首頁 banner 輪播＋最新消息列表，含 article 圖文展開）** 已落地；待辦為其餘業務頁 |
