...
| SaaS 化 ①：RLS 租戶隔離 | `2026-09-20-rls-tenant-isolation-plan.md` | 租戶隔離啟用（SaaS spec 第①步） | 🟡 計畫已備、未開工 | 10 tasks／67 steps：DSN 分流、`app_rw` 非 owner、14 個 policy 補 `WITH CHECK` ＋三張漏網表、請求層租戶交易（`dbtenant`）、依域分批 ENABLE、跨租戶端到端探針。**② platform 域＋權益守衛、③ 生命週期與平台營運六頁工具（`platform-console/`）待寫計畫** |
