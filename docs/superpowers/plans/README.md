...
| 08-dispatch | `backend/2026-08-17-backend-08-dispatch-plan.md` | 派車看板 | 🟡 部分 | 後端約 90%：`DispatchService` 4 RPC（Assign／Confirm／Cancel／WatchBoard）＋派車通知＋提交後發佈（程序內直投；跨 replica 另案）＋重印警告；Web 看板頁已落地（`e3ed769`：日期過濾前置＋車次欄 DnD＋批次確認＋取消派車＋串流訂閱）；跨 replica Valkey pub/sub 與輪詢降級待 |
