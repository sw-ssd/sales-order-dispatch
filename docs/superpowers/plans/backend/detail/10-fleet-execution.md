# Backend Detail — fleet 執行層（10-fleet-execution）

> 版本：v0.1.0（2026-09-17）
> 依據：決策 **D32**、需求規格 `docs/superpowers/specs/1.0-requirements/fleet-execution/spec.md`、規劃整合 `docs/PLANNING_OVERVIEW.md`。
...
- **分層**:`backend/internal/domain/fleet/` 採 **handler → usecase → repository** 三層 + `register.go`（含 `transformation.go` DTO 轉換）;層間以 interface 解耦、經 `internal/server/domains.go` 的 `InitDomains()` 每 domain 一行組裝掛載（新增 domain 只動此檔）。
...
- **組態**:逐檔 struct + `envconfig` tag(`backend/config/fleet.go` 放 `OSRM_HOST`、geocode 端點等;`config.New()` 聚合),不散落 env。
...
- `fleets` / `vehicles` / `drivers`：均帶 `company_id` + `department_id` + `deleted_at`（軟刪除，D10）。`vehicles.plate_no` 部門內唯一（部分唯一索引）。`drivers.user_id` 關聯既有 `users`。
...
- `fleet_deliveries`：承接 `route_id`（車次）的執行單元；`driver_assigned_uuid`/`vehicle_assigned_uuid`/`assigned_by`/`status`（pending/dispatched/in_progress/completed/cancelled）/`started_at`/`completed_at`/`version`。
- `fleet_delivery_events`：執行軌跡（event_type + payload + created_at）。
- `proofs`：POD；`delivery_id`/`type`（photo/signature/scan）/`file_asset_id`（D17）/`captured_at`。

## 共通規則（沿用 00-index §3，另加下列）

- **RLS**：fleet 表 policy 以 `app.current_department_id` + `app.current_data_scope` 過濾（department 級;company_admin 看全部門、super 全公司）。runtime 依 Ent 官方 RLS 做法：`sql.WithStringVar(ctx, "app.current_department_id", ...)` / `WithStringVar` + `data_scope`,由 tenant middleware 注入。
...
- **檔案**：`ent/schema/fleet.go`、`vehicle.go`、`driver.go`;migration:`database/goose/xxx_fleet_master.sql`(含 `ENABLE ROW LEVEL SECURITY` + `CREATE POLICY fleet_dept_isolation`);`proto/v1/fleet.proto`(`FleetService`)。
- **介面**：`FleetService.CreateVehicle` / `CreateDriver` / `ListVehicles`(分頁 + filter)/ `ListDrivers` / `SoftDeleteVehicle|Driver`。
...
- **檔案**：`ent/schema/service_area.go`、`zone.go`;migration(PostGIS Polygon);`proto/v1/fleet.proto`(service_areas/zones 於 `FleetService`)。
- **介面**：`FleetService.CreateServiceArea` / `ListZones` / `ZoneBelongs`(檢查點是否落在 zone)。
...
- **錯誤處理**：geocode 失敗不回丟建檔錯誤;於 fleet 路線計算時才提示缺座標。
- **驗收**：shipping 地址 geocode 成功寫座標;geocode 失敗仍建檔且 `location` 空。
- **相依**：`10.3` 提供 fleet 路線(10.7)所需的終點座標。

### 10.4 車次 ↔ 車輛 / 司機指派（AssignmentService）

- **目標**：後台把車次指派實際車輛/司機,粒度在 `fleet_delivery`。
- **檔案**：`ent/schema/fleet_delivery.go`;`internal/domain/fleet`(handler/usecase/repository/register);`proto/v1/fleet.proto`(`AssignmentService.AssignRouteDelivery` / `ReassignVehicle`)。
- **介面**：`AssignmentService.AssignRouteDelivery(route_id, driver_uuid, vehicle_uuid, version)`。
- **實作邏輯**：由 1.0 已裝單完成的 `route_id` 建立/更新 `fleet_delivery`;寫 `driver_assigned_uuid`/`vehicle_assigned_uuid`/`assigned_by`;樂觀鎖比對 `version`(衝突重試/拒絕);完成後發 `order.driver_assigned`(Valkey pub/sub 通知司機 App)。**非車次↔車 1:1**,可重指派(version 遞增、寫稽核)。
...
- **目標**：司機執行配送與簽收。
- **檔案**：`ent/schema/proof.go`、`fleet_delivery.go` 狀態欄位;`proto/v1/fleet.proto`(`DeliveryService.Start / Complete / Cancel`);POD 走檔案資產(D17)。
- **介面**：`DeliveryService.Start(delivery_id)` / `Complete(delivery_id, proof)` / `Cancel(delivery_id, reason)`。
- **實作邏輯**：狀態機 `pending→in_progress→completed`,`cancelled`(含原因);完成時寫 `proofs`(photo/signature/scan + file_asset_id);狀態異動 + 稽核 + `fleet_delivery_events` 同一交易;OpenFGA 判定操作者為被指派司機。
...
- **檔案**：`third_party/osrm`(client,`OSRM_HOST` 設定);`internal/domain/fleet/routing.go`;`proto/v1/fleet.proto`(`RoutingService.GetRoute`)。
...
### 10.8 OpenFGA 授權接入（fleet 資源）

- **目標**：fleet 資源納入 OpenFGA 決策。
...
- **介面**：middleware 對受保護 fleet RPC `Check`;list 用 `list-objects`。
- **實作邏輯**：型別 = `company`/`department` 租戶 + `role`/`group`/`system`;資源 `driver`/`vehicle`/`fleet_delivery` 以租戶 parent 邊 + userset rewrite;指派/建檔時寫 tuple(經事件流)。
- **錯誤處理**：`permission_denied`(Check false)。
- **驗收**：被指派司機可操作其 delivery、他人 403;`list-objects` 對所屬部門回正確集合。
- **相依**：本子功能為 fleet 全部 RPC 的授權前置(bootstrap 於 `Server.Init()`)。

---

### 10.9 每單送達狀態回寫（A,1.0）

- **目標**：`fleet_delivery` 完成時把送達回寫到該車次所載每筆 `sales_order`（標 `delivered`）。
- **檔案**：`internal/domain/fleet/delivery.go`（完成流程內）；update `ent`（`sales_order.status` 沿用既有狀態機）；`proto/v1/sales_order.proto`（查詢帶 `delivered_date`）。
...
- **檔案**：`internal/domain/fleet/geofence.go`；`third_party/geocode`（已含空間計算）；事件寫 `fleet_delivery_events`。
...
### 10.11 fleet 通知接 D16（A,1.0）

- **目標**：reuse 既有通知（D16：FCM+站內、範本/裝置管理），不另建一套。
- **檔案**：`internal/domain/fleet/notify.go`（訂閱 fleet 事件 → 叫 `notifications` domain）；沿用 D16 範本。
...
- **檔案**：`internal/domain/fleet/driver.go`;`internal/authz`(openfga model 增 `driver` relation);`config/fleet.go`。
...
- **目標**：`fleet_delivery` 展開為依 `delivery_sequence` 排序的停點(客戶/地址/訂單/座標),供司機 App 逐站執行。
- **檔案**：`ent`(或由 `fleet_deliveries` + `sales_order` join 產生,不加持久表,簡版以查詢產出)。
...
自動派單、VRP 多車次優化、現場拒收/部分交付(串 D25)、ETA 異常通知、customer 端 live map、車輛狀態/保養、司機 App 背景 GPS/離線、每單 tracking 狀態鏈、位置軌跡回播/稽核、fleet 報表/績效、多公司 fleet 視圖、出庫/裝車確認、司機簽到離班、重簽/作廢 POD、同車次多車拆分、送達時窗、異常通報——均列 `fleet-execution/spec.md` §1.1+ 延伸,1.0 不做。

## 整合測試重點（D21）

RLS 部門隔離(fleet 表)、OpenFGA Check/list-objects、指派樂觀鎖並發、狀態機轉移、POD 上傳 + 稽核、地理編碼失敗不擋建檔、Valkey 串流跨 replica 送達。
