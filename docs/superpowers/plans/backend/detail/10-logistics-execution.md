...
> - **未落地**：`logistics_teams` 聚合、`service_areas`/`zones`（PostGIS）、`positions` GPS 串流（10.5、10.7、10.10 geofence）、10.13 停點清單、部分交付語意（1.1+）、uuid 主鍵、department#member/#admin 成員 provision（目前 instance 判決靠 assigned_by/driver 邊;成員邊產製另案）。
