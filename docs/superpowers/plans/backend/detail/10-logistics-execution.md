...
> - **未落地**：`logistics_teams` 聚合、`service_areas`/`zones`（PostGIS）、`positions` GPS 串流（10.5、10.7、10.10 geofence）、**10.9 送達回寫**（完成時回寫車次所載每筆 `sales_order`）、**10.11 通知接 D16**（指派/完成推播）、10.13 停點清單、uuid 主鍵、department#member/#admin 成員 provision（目前 instance 判決靠 assigned_by/driver 邊;成員邊產製另案）。
