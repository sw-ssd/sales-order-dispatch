...
> - **未落地**：`logistics_teams` 聚合、`service_areas`/`zones`（PostGIS）、`positions` GPS 串流（10.5–10.7）、`logistics_delivery_events` 軌跡、狀態機（10.6,pending/in_progress/... 僅建列預設 pending）、POD（10.10）、`logistics_delivery_events`/proofs 欄位綱要其餘、uuid 主鍵、department#member/#admin 成員 provision（目前 instance 判決靠 assigned_by/driver 邊;成員邊產製另案）。
> - **測試**：model 契約 5 子測試（`internal/authz/openfga/logistics_test.go`）、服務 3 測試（sqlite,含樂觀鎖與閘門）、跨部門/本人隔離整合探針（app_rw + 真 RLS + 記憶體引擎,`logistics_rls_integration_test.go`）全綠。
