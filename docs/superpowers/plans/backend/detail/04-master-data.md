...
> 🔒 **本子功能留待 07 開工（2026-09-18 決策）**：`promo_tag_ids` 交叉驗證依賴 `promo_tags` 表（07 §4.3.5 僅建資料層）尚不存在;`preferred_delivery_days` 長度 6 驗證亦同批待補。WS1 全數延後至 07。
...
- **驗收**:
  - [x] 同一客戶可新增 `shipping` 與 `billing` 地址並各自查得。
  - [x] 多筆 `shipping` 中設定某筆為預設後,同類型其餘地址自動失去預設;同類型至多一筆預設。
  - [x] 刪除地址後不再出現於預設查詢,歷史單據顯示不受影響。
...
- **驗收**:
  - [x] 客戶可有多筆聯絡人;設定預設後至多一筆 `is_default = true`。
  - [x] 首筆新增自動成為預設;刪除預設聯絡人後清單呈現無預設。
  - [x] 軟刪除聯絡人不影響歷史單據顯示。
...
- **驗收**:
  - [x] 甲、乙部門各自建倉,互不可見、商品與單據不可跨部門選用。
  - [x] 軟刪除倉別後新單據選項消失,既有引用正常顯示。
...
- **錯誤處理**: 同 3.4.1。
- **驗收**:
  - [x] 部門各自維護車次,跨部門不可見不可選。
  - [x] 軟刪除車次不再出現於新單據選項,歷史訂單車次名稱仍顯示。
...
- **驗收**:
  - [x] 處理規格帶兩旗標(可同時 true)與 `attributes`,可供商品多對多關聯(案例 1:一商品多規格)。
  - [x] `kind` 開放集(metadicts 背書),部門可新增 `processing_kind` 字值不需改碼。
  - [x] 軟刪除規格不可再被新關聯引用,既有商品查詢之規格清單自動排除。
...
- **錯誤處理**: 同 3.4.1。
- **驗收**:
  - [x] 商品可指定本部門分類並以分類篩選列表。
  - [x] 軟刪除分類後商品不可新設該分類,既有商品分類名稱仍顯示。
