...
| 2 | 範本渲染 + 通知記錄/已讀 API（細部 4.3.2–4.3.3） | ✅ 完成（後端＋Web 通知中心 `b193544`；App 畫面待） |
...
**實作範圍**：後端約 85%（FCM 實裝＋派車 adapter 待）＋Web 通知中心 `b193544`（清單/未讀數/狀態與通道篩選/單列與整頁標記已讀；刻意不做「全部已讀」）；App 畫面待。
...
*最後更新：2026-09-22（07-notifications 後端＋Web 通知中心 `b193544` 對齊；FCM 實裝＋派車 adapter 待、App 畫面待）*
