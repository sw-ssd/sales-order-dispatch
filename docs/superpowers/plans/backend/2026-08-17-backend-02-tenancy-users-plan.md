...
**說明**：`public_info` / `capabilities` 欄位與序列化已就緒。**未含** Logo 檔案上傳（FileStore）、Branding、公開發現端點（`/api/v1/companies/public/{identifier}`）。

- [x] **Step 1: PublicInfo/capabilities 欄位** — company schema + 序列化
- [ ] **Step 2: Logo 上傳** — FileStore（04 計畫）＋ logo_file_id
