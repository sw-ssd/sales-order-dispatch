...
> **現況註記（2026-09-23 盤點,以實際程式碼為準,checkbox 已依現況全勾）**：10 tasks 均已落地。三處交付與計畫文字的有意分歧：
> 1. **Task 4**：`model.fga` 以 `third_party/openfga/model.go` 的 `modelDSL` Go 字串交付（go:embed 工具鏈 bug,內容同 DSL,與 repo 同版本控管）。
> 2. **Task 7**：核心表 RLS 由後續 Plan A（2026-09-20 RLS 計畫,00022 起,現況 32 張 ENABLE+FORCE ＋ `dbtenant` 請求層交易）**超集**落地,非本計畫的 00007 單檔。
> 3. **Task 9**：`internal/authz/casl/` **保留**——僅 condition AST／ParseConditions／FieldRegistry（role_permissions 條件解析與驗證,不做授權決策）;退場的是 Casbin 依賴、CASL 執行面（AccessibleFilter/Can）與死開關 `CASL_ENFORCEMENT_ENABLED`（2026-09-23 移除）。
...
- [x] **Step 1: 寫 migration 00005（core schema，對齊 ent schema）**
...
- [x] **Step 2: 寫 migration 00006（seed roles/permissions）**
...
- [x] **Step 4: 寫並跑 seed/migration 測試**
...
- [x] **Step 5: 跑測試確認通過 + 更新 .env.example**
...
- [x] **Step 3: 跑測試確認失敗** → Run `go test ./third_party/openfga/... -run WriteModel -v`。Expected: FAIL。

- [x] **Step 4: 實作 model 載入 + 寫入**

`openfga.go`：以 `typesystem`/`WriteAuthorizationModel` 將 `model.fga` 轉 `TypeDefinition` 寫入。

- [x] **Step 5: 跑測試確認通過 + Commit**
...
- [x] **Step 2: 跑測試確認失敗** → Run `go test ./internal/authz/openfga/... -v`。Expected: FAIL。

- [x] **Step 3: 實作封裝**

`internal/authz/openfga/openfga.go`：包 `*openfga.Client` 的 `Check`/`ListObjects`/`WriteTuple`/`DeleteTuple`。

- [x] **Step 4: 改 access.go 移除 CASL 執行面**
...
- [x] **Step 5: 跑測試確認通過** → Run `go test ./internal/authz/... -v`。Expected: PASS。
...
- [x] **Step 2: 跑測試確認失敗** → Run `go test ./internal/server/... -v`。Expected: FAIL。

- [x] **Step 3: 實作 middleware 接線**
...
- [x] **Step 4: 跑測試確認通過** → Run `go test ./internal/server/... -v`。Expected: PASS。
...
- [x] **Step 2: 延伸 rls_test**

`rls_test.go`：對各 data_scope 驗證 `RLSStatements` 產出含對應 `SET LOCAL app.current_*` 且順序決定性。

- [x] **Step 3: 跑測試確認通過** → Run `go test ./internal/auth/... -v`。Expected: PASS。
...
- [x] **Step 2: 跑測試確認失敗** → Run `go test ./internal/services/... -run OpenFGA -v`。Expected: FAIL。
...
- [x] **Step 4: 跑測試確認通過** → Run `go test ./internal/services/... -v`。Expected: PASS。
...
- [x] **Step 1: 改 GetAbility 為 OpenFGA 驅動**
...
- [x] **Step 3: 跑測試確認通過** → Run `go test ./internal/domain/auth/... -v`。Expected: PASS。

- [x] **Step 4: 刪除 Casbin/CASL code + 移除依賴**
...
- [x] **Step 1: 寫整合測試**（無 Postgres 自動 skip）
- [x] **Step 2: 跑測試確認通過** → Run `go test ./internal/authz/... -tags integration -v`。Expected: PASS（無 DSN 時 SKIP）。
- [x] **Step 3: Commit** `git add -A backend && git commit -m "test(backend): OpenFGA+RLS 授權隔離整合測試(D21)"`
