# fleet-execution 需求規格

> 來源：2026-09-17 Fleetbase 整合（決策 D32）。Fleetbase 側整合決策見 Fleetbase 倉庫 `docs/FLEETBASE_物流平台重建_PLAN.md`。本檔為 fleet 執行層的需求層規格（Requirement + Scenario），細部實作見 `docs/superpowers/plans/backend/detail/10-fleet-execution.md`。
...
系統 SHALL 提供 fleet 主檔 `fleets` / `vehicles` / `drivers` 的建立、查詢、更新與軟刪除；三者 MUST 帶 `company_id` 與 `department_id`，資料範圍依使用者 data_scope（staff/dept_admin 限本部門、company_admin 限所屬公司全部門、super 全公司）。`vehicles` MUST 記錄 `plate_no`、`vehicle_type`、`capacity`；`drivers` MUST 關聯既有 `users`（司機為一種使用者）並記錄 `name`、`phone`、`assigned_vehicle_id`。業務唯一性（如 `plate_no`）配合軟刪除部分唯一索引（D10）。
...
- **THEN** 僅回傳部門乙的 vehicles/drivers/fleets，不含部門甲任何資料
...
系統 SHALL 提供 `service_areas` 與 `zones`（部門級），記錄地理邊界（PostGIS `geography(Polygon)`；若不可用 fallback GeoJSON 文字欄位），用於後續派單與區域檢核。資料範圍與 fleet 主檔相同。
...
系統 SHALL 於 `customer_addresses`（`type=shipping`）記錄座標 `location geography(Point)`，於建立/更新地址時進行地理編碼（geocode）；供 fleet 路線 / ETA（OSRM）使用。**不另建立 Fleetbase 式 places 表**；地址簿仍為客戶子資源（master-data）。地理編碼失敗時，地址 MUST 仍可建立（僅無座標），並於派單路線計算時提示缺座標。
...
- **AND** 當該地址被納入 fleet 路線計算時，系統提示缺座標（`invalid_argument` 或前端警示）

### Requirement: 車次 ↔ 車輛 / 司機指派（後台，粒度在 fleet_delivery）

系統 SHALL 提供 `AssignmentService`，由後台把一個已裝單完成的車次（`route_id`）指派實際車輛與司機，寫入 `fleet_deliveries`（每筆帶 `driver_assigned_uuid` / `vehicle_assigned_uuid`、`assigned_by`、`version`）。指派以樂觀鎖比對 `version`，並發衝突時拒絕並提示。**非固定「車次↔車」1:1**：一車一司機可服務多車次、可重指派（重指派 `version` 遞增）。自動派單（距離最近司機）排 v1 之後，v1 僅手動。

#### Scenario: 指派成功

- **WHEN** 後台對車次 R1 的 `fleet_delivery` 指派司機 D 與車輛 V，且 `version` 一致
- **THEN** `fleet_deliveries` 寫入 `driver_assigned_uuid`/`vehicle_assigned_uuid`/`assigned_by`、`version` 遞增
- **AND** 司機 App 收到該車次執行清單

#### Scenario: 並發指派樂觀鎖衝突

- **WHEN** 兩請求同時以相同 `version` 指派同一 `fleet_delivery`
...
系統 SHALL 提供 `DeliveryService` 讓司機對指派給自己的 `fleet_delivery` 執行開始（started）、完成（completed）與取消（cancelled）；完成時可上傳簽收證明 **POD**（`proofs`，`type` 為 `photo` / `signature` / `scan`）。POD 影像走 1.0 檔案資產（D17），操作寫稽核（D18）。狀態異動寫 `fleet_delivery_events` 事件軌跡。

#### Scenario: 完成配送並上傳簽收

- **WHEN** 司機完成該 `fleet_delivery` 並上傳 photo POD
- **THEN** `fleet_deliveries.status=completed`、`proofs` 記錄型別與檔案 asset id、寫稽核與事件軌跡
- **AND** 通知/看板反映完成狀態

#### Scenario: 非本人不可操作

- **WHEN** 非被指派的司機嘗試開始/完成某 `fleet_delivery`
...
### Requirement: NetSuite 不接 fleet

fleet 執行層 MUST NOT 與 NetSuite 同步或耦合；fleet 領域無任何 NetSuite 欄位或同步邏輯。

#### Scenario: fleet 領域無 NetSuite 依賴

- **WHEN** 檢查 fleet 領域 schema 與程式碼
- **THEN** 無 NetSuite 欄位、無 NetSuite client 呼叫、無同步 job

### Requirement: 每單送達狀態回寫（A,1.0）

系統 SHALL 在 `fleet_delivery` 完成時，將送達結果**回寫到該車次所載的每筆 `sales_order`**（標 `delivered`），使店家 / 客戶可看到每一單的送達狀態；回寫與 `fleet_delivery` 狀態異動、稽核同事務（D18）。未覆蓋到的訂單（部分交付語意）在 1.1+ 處理。

#### Scenario: 車次完成回寫逐單送達

- **WHEN** 司機完成該 `fleet_delivery`（含 POD）
...
### Requirement: fleet 通知接 D16（A,1.0）

fleet 領域的通知（新任務指派推司機、送達推店家 / 主責業務）SHALL 走既有通知基礎設施（D16：FCM + 站內、範本/裝置管理），不另建一套通知。

#### Scenario: 指派推司機、送達推客戶

- **WHEN** 後台指派 `fleet_delivery` 或司機完成送達
...
- **THEN** 取得 JWT 且 OpenFGA/RLS 僅允許其部門（data_scope=department）內、且 `assignee` 為自己的 `fleet_delivery`

### Requirement: 配送停點 / waypoint 清單（1.0 簡版）

系統 SHALL 提供「逐站執行清單」：每個 `fleet_delivery` 展開為依 `delivery_sequence` 排序的**停點（waypoint）**(客戶 / 送貨地址 / 訂單明細),供司機 App 導航與逐一執行；停點與訂單/地址座標關聯（地址座標見 master-data）。

#### Scenario: 司機取得逐站清單

- **WHEN** 司機載入被指派的 `fleet_delivery`
...
- **THEN** 上報位置並完成 `fleet_delivery`、回寫 `sales_order`、走 D16 通知

## 1.1+ 延伸（不在 1.0 範圍,v1 後再做）

以下 fleet 能力納入 1.1+（或另行評估），不擠入 1.0 fleet 執行層：
...
- fleet 報表與績效分析（出貨量、司機完成率）
- 多公司 fleet 視圖（super 跨公司看車隊）
- 出庫 / 裝車確認（銜接 1.0 揀貨單）
- 司機簽到 / 離班 / 交接
- 重簽 / 作廢 POD（簽收時效操作）
- 同車次多車拆分（車次單過多拆多趟）
