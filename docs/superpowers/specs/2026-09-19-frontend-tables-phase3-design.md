...
**D12 已知刻意變更：清單載入失敗時不保留上一頁列**（3A 波複審 I1 的殘留裁定）
- 行為：換頁／換篩選條件的請求**失敗**時，清單**不再保留上一頁的列**，改為「錯誤 banner ＋ 空表」（空狀態列本身仍不顯示——空狀態條件帶 `!query.isError`）。
- 對比：3A 之前的 BASE 會保留上一頁的列（換頁失敗時畫面上仍是舊資料）。
- 理由：避免「舊列 ＋ 新頁碼 ＋ 錯誤」的誤導畫面——該頁確實沒有資料，顯示舊列卻標著新頁碼會讓人以為資料已換頁；且要保留舊列就得在頁面再存一份結果副本，與 `queries.ts` 的「頁面不得留第二份結果」契約（單一真相）牴觸。
- 影響：使用者看到的是**錯誤 banner ＋ 空表**，而非舊列；`placeholderData: (prev) => prev` 仍有效（載入中不閃空），失敗時 `isPlaceholderData` 為 `false` 才切成空表。
- 後續：若日後要 sticky rows，須以查詢快取的單一真相實作（例如把「最後一次成功的結果」放進 query client 層），不得在頁面另存。
