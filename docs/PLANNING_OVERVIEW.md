...
> 核心狀態：**monorepo；backend 01 auth、02 tenancy/users、03 metadicts/audit、04 master-data（含客戶專屬商品/檔案資產/QR 兌換）、05 sales-orders（後端 API＋狀態機＋取號）、09 printing 全數落地；06 returns（後端＋Web 退貨頁 `549933c`）、07 notifications 後端亦已落地（07 約 85%，FCM 為 Fake sender）、08 dispatch 後端約 90%＋Web 看板已落地；fleet 未開工；frontend UI 四階段完成並落地 8 頁（使用者/客戶/訂單/商品/退貨/派車看板/車次主檔/單據列印）、app 僅骨架；未部署**
...
*最後更新：2026-09-22（核心狀態行對齊 06/07/08 與 8 個 Web 頁）*
