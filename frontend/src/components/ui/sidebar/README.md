...
| `SidebarRail` | — | 側欄內緣熱區，純 toggle、不進 tab 順序 |
| `SidebarInset` | — | 主要內容區 |
...
1. **側欄關閉時仍可 Tab**：交給 Ark `drawer`。關閉時 Presence 把 `Positioner`／`Content` 標成
   `hidden`，內容離開無障礙樹與 tab 順序（測試以 `queryByRole("link")` 為 null 守住），Esc 可關閉。
