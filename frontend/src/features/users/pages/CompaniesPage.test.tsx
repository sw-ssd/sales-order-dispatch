...
describe("<CompaniesPage> 表格（TanStack Table，manual 分頁）", () => {
  ...
  it("表頭與 columns 一致：六欄（名稱/識別碼/統一編號/狀態/ID/操作），資料列的格子數相同", async () => {
    ...
    expect(tableRows()[0].querySelectorAll("td")).toHaveLength(6);
  });
...
describe("<CompaniesPage> 表頭排序（伺服器端）", () => {
  ...
  it("第三次點同一欄＝取消排序：請求回 sort 空字串、aria-sort 回 none、指示器消失，且回第 1 頁", async () => {
    ...
    await waitFor(() => expect(listCompaniesSpy).toHaveBeenCalledTimes(5));
    expect(listCompaniesSpy).toHaveBeenLastCalledWith(payload(1, "", false));
    await settle();
    expect(listCompaniesSpy).toHaveBeenCalledTimes(5);
