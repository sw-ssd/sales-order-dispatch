...
// TestFleetDeliveryTenantAndAssigneeEdges 是 fleet model(D32/10.8)的契約測試:
//
...
func TestFleetDeliveryTenantAndAssigneeEdges(t *testing.T) {
	t.Parallel()
	...
	write("company:1#member", "company", "fleet_delivery:f1")
	write("department:100#member", "department", "fleet_delivery:f1")
	write("department:100#admin", "manager", "fleet_delivery:f1")
	write("driver:d1#assignee", "driver", "fleet_delivery:f1")
	write("user:manager1", "assigned_by", "fleet_delivery:f1")
	...
	t.Run("被指派司機可讀寫自己的 delivery", func(t *testing.T) {
		if !check("user:driver1", "can_read", "fleet_delivery:f1") {
			t.Fatal("被指派司機應可讀")
		}
		if !check("user:driver1", "can_write", "fleet_delivery:f1") {
			t.Fatal("被指派司機應可寫(Start/Complete/Cancel 路徑)")
		}
	})
	t.Run("指派人可讀寫", func(t *testing.T) {
		if !check("user:manager1", "can_read", "fleet_delivery:f1") {
			t.Fatal("assigned_by 應可讀")
		}
		if !check("user:manager1", "can_write", "fleet_delivery:f1") {
			t.Fatal("assigned_by 應可寫")
		}
	})
	t.Run("部門管理者的寫權限走 department#admin → manager 邊", func(t *testing.T) {
		if !check("user:manager2", "can_write", "fleet_delivery:f1") {
			t.Fatal("dept_admin(非指派人)應可寫(manager 邊)")
		}
		if !check("user:manager2", "can_read", "fleet_delivery:f1") {
	...
	t.Run("部門成員可讀、未指派者 403", func(t *testing.T) {
		...
		if !check("user:staff1", "can_read", "fleet_delivery:f1") {
			t.Fatal("部門成員應可讀")
		}
		if check("user:staff1", "can_write", "fleet_delivery:f1") {
			t.Fatal("未被指派的成員不得寫(寫限 dept_admin/被指派者/指派人)")
		}
		if check("user:stranger", "can_read", "fleet_delivery:f1") {
			t.Fatal("無任何邊的使用者不得讀(他人 403)")
		}
	})
	t.Run("重指派:判決跟著 tuple 走", func(t *testing.T) {
		if err := e.DeleteTuple(ctx, "driver:d1#assignee", "driver", "fleet_delivery:f1"); err != nil {
			t.Fatalf("DeleteTuple(舊指派): %v", err)
		}
		write("driver:d2#assignee", "driver", "fleet_delivery:f1")
		if check("user:driver1", "can_read", "fleet_delivery:f1") {
			t.Fatal("被替換的司機不得再讀")
		}
		if !check("user:driver2", "can_read", "fleet_delivery:f1") {
			t.Fatal("新司機應可讀")
		}
		if !check("user:driver2", "can_write", "fleet_delivery:f1") {
			t.Fatal("新司機應可寫")
		}
	})
}
