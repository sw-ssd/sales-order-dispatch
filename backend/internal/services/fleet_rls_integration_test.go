...
// TestIntegrationFleetCrossDeptAndSelfIsolation D32 整合探針(真 PG + app_rw + 真 RLS +
...
//  3. 跨部門 RLS:scope=dept A 直接 SQL 讀 dept B 的 fleet_deliveries → 0 列;
...
func TestIntegrationFleetCrossDeptAndSelfIsolation(t *testing.T) {
	...
	for _, q := range []struct {
		...
		{"app_rw CRUD", `SELECT has_table_privilege('app_rw', 'fleet_deliveries', 'SELECT')
			AND has_table_privilege('app_rw', 'fleet_deliveries', 'INSERT')
			AND has_table_privilege('app_rw', 'fleet_deliveries', 'UPDATE')
			AND has_table_privilege('app_rw', 'fleet_deliveries', 'DELETE')`},
		{"app_rw 序列", `SELECT has_sequence_privilege('app_rw', 'fleet_deliveries_id_seq', 'USAGE')
			AND has_sequence_privilege('app_rw', 'fleet_drivers_id_seq', 'USAGE')
			AND has_sequence_privilege('app_rw', 'vehicles_id_seq', 'USAGE')`},
		{"ENABLE", `SELECT relrowsecurity FROM pg_class WHERE relname = 'fleet_deliveries'`},
		{"FORCE", `SELECT relforcerowsecurity FROM pg_class WHERE relname = 'fleet_deliveries'`},
		{"policy 存在", `SELECT count(*) = 3 FROM pg_policies
			WHERE tablename IN ('fleet_drivers', 'vehicles', 'fleet_deliveries')
	...
	co := adminDB.Company.Create().SetName("Fleet 探針公司").SetIdentifier("FLP-1").
	...
	fgaClient, err := ofga.NewMemory(ctx, "fleet-rls-probe")
	if err != nil {
	...
	fleetServer := func(id authz.Identity, scope auth.RLSScope) salesorderv1connect.FleetServiceClient {
		t.Helper()
		mux := http.NewServeMux()
		RegisterFleetService(mux, appClient)
	...
	{
		rpc := fleetServer(idOf(uItoa(mgrA), "dept_admin", deptA.ID), scopeDept(deptA.ID, uItoa(mgrA)))
	...
	{
		rpc := fleetServer(idOf(uItoa(mgrB), "dept_admin", deptB.ID), scopeDept(deptB.ID, uItoa(mgrB)))
		d, err := rpc.CreateDriver(ctx, connect.NewRequest(&v1.CreateDriverRequest{
	...
	myList := func(did int, uid int, want int) {
		t.Helper()
		rpc := fleetServer(idOf(uItoa(uid), "staff", did), scopeDept(did, uItoa(uid)))
	...
	var bRows int
	if err := tx.QueryRow(`SELECT count(*) FROM fleet_deliveries WHERE department_id = $1`,
	...
	if _, err := tx.Exec(
		`INSERT INTO fleet_deliveries (company_id, department_id, route_id, assigned_by)
	...
	ok, err := engine.Check(ctx, "user:"+uItoa(mgrB), "can_write", fleetObj(mustAnnID(t, deliveryA)))
	if err != nil {
