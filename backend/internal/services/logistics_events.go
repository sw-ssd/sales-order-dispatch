// Package services 的 logistics tuple 同步(D32/10.8)。
//
...
// managedLogisticsRelations 為本模組管理的 relation 集合(reconcile 只動這些)。
var managedLogisticsRelations = map[string]bool{
	"company": true, "department": true, "manager": true,
	"driver": true, "assigned_by": true, "assignee": true,
}

// logisticsObj / driverObj 為 tuple 物件定位(D32 model 型別名)。
func logisticsObj(id int) string { return "logistics_delivery:" + strconv.Itoa(id) }
...
// reconcileLogisticsTuples 把 obj 的現存 managed tuples 對帳到 desired(刪 stale、補缺漏)。
func reconcileLogisticsTuples(ctx context.Context, e *authzopenfga.Engine, obj string, desired [][3]string) error {
	...
	for _, t := range existing {
		if t[2] != obj || !managedLogisticsRelations[t[1]] {
			continue
		}
...
// afterCommitLogisticsTuples 註冊提交後的 tuple 對帳(outbox 簡化語意:失敗僅記日誌)。
// e/obj/desired 皆在提交前取定,closure 只讀快照。
func afterCommitLogisticsTuples(ctx context.Context, e *authzopenfga.Engine, obj string, desired [][3]string) error {
	return dbtenant.AfterCommit(ctx, func(ctx context.Context) error {
		if err := reconcileLogisticsTuples(ctx, e, obj, desired); err != nil {
			log.Printf("logistics: tuple 對帳失敗(%s,重指派/重建可修復): %v", obj, err)
		}
		return nil
	})
}
