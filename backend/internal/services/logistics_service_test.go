...
// logisticsFixture 為一組部門 + 司機/管理者 + 車次(sqlite;每次獨立記憶體庫)。
type logisticsFixture struct {
	...
	driver1ID int // staff,有 logistics_drivers 列
	driver2ID int
	routeID   int
}

// openLogisticsFixture 開獨立 db、起記憶體 OpenFGA 引擎、建基礎資料。
func openLogisticsFixture(t *testing.T) *logisticsFixture {
	t.Helper()
	f := &logisticsFixture{}
	db := enttest.Open(t, "sqlite3", "file:logistics"+logisticsDBSeq()+"?mode=memory&cache=shared&_fk=1")
	t.Cleanup(func() { _ = db.Close() })
	f.db = db

	client, err := ofga.NewMemory(context.Background(), "logistics-svc-test-store")
	if err != nil {
	...
	f.coID = db.Company.Create().SetName("Logistics 公司").SetIdentifier("FLT-1").SetStatus("active").SaveX(ctx).ID
	f.deptID = db.Department.Create().SetName("Logistics 部").SetCompanyID(f.coID).SaveX(ctx).ID
...
// logisticsDBSeq 讓每案獨立記憶體庫(同 dsn 會共用)。
var logisticsSeq int

func logisticsDBSeq() string {
	logisticsSeq++
	return strconv.Itoa(logisticsSeq)
}

// newLogisticsServer 以指定身分 + 記憶體引擎掛真 handler(與生產同構:
// requestid/dbtenant 攔截器 + authz 身分/引擎注入)。
func (f *logisticsFixture) newLogisticsServer(t *testing.T, id authz.Identity) salesorderv1connect.LogisticsServiceClient {
	t.Helper()
	mux := http.NewServeMux()
	RegisterLogisticsService(mux, f.db)
...
// logisticsIdentity 造身分(Roles 走內建繼承展開;dept 帶上)。
func logisticsIdentity(role string, uid, cid, did int) authz.Identity {
...
// TestLogisticsCreateAndAssignTuplesSync D32 全鏈( sqlite 單測版):
// 建司機/車輛 → 指派車次 → tuple 經 AfterCommit 對帳 → 被指派司機
// 以 instance 級 Check 通過;重指派樂觀鎖 + 判決移交。
func TestLogisticsCreateAndAssignTuplesSync(t *testing.T) {
	ctx := context.Background()
	f := openLogisticsFixture(t)
	rpc := f.newLogisticsServer(t, logisticsIdentity("dept_admin", f.managerID, f.coID, f.deptID))
	...
	for _, rel := range []string{"can_read", "can_write"} {
		if ok, err := f.engine.Check(ctx, "user:"+uItoa(f.driver1ID), rel, "logistics_delivery:"+delID); err != nil || !ok {
			t.Fatalf("被指派司機 %s 應通過,ok=%v err=%v", rel, ok, err)
		}
	}
	// 無關者 deny。
	if ok, _ := f.engine.Check(ctx, "user:999", "can_read", "logistics_delivery:"+delID); ok {
		t.Fatal("無關者不得讀取")
	}
	...
	if ok, _ := f.engine.Check(ctx, "user:"+uItoa(f.driver1ID), "can_read", "logistics_delivery:"+delID); ok {
		t.Fatal("被替換的司機不得再讀(tuple 對帳已移交)")
	}
	if ok, _ := f.engine.Check(ctx, "user:"+uItoa(f.driver2ID), "can_read", "logistics_delivery:"+delID); !ok {
		t.Fatal("新司機應可讀")
	}
}

// TestLogisticsCreateGuards 建檔守衛:車牌重複 already_exists、跨公司參照 not_found、
// 非管理角色(staff 無 logistics write) permission_denied。
func TestLogisticsCreateGuards(t *testing.T) {
	ctx := context.Background()
	f := openLogisticsFixture(t)

	deptAdmin := f.newLogisticsServer(t, logisticsIdentity("dept_admin", f.managerID, f.coID, f.deptID))
	...
	// staff 無 logistics write(rolePolicy) → middleware 之外的 requireScope 拒絕。
	staff := f.newLogisticsServer(t, logisticsIdentity("staff", f.driver1ID, f.coID, f.deptID))
...
// TestLogisticsListMyDeliveriesGates 10.12/10.8 閘門:
// 非司機身分拒絕;司機只見自己被指派的;tuple 缺席的列被 instance 級過濾。
func TestLogisticsListMyDeliveriesGates(t *testing.T) {
	ctx := context.Background()
	f := openLogisticsFixture(t)
	deptAdmin := f.newLogisticsServer(t, logisticsIdentity("dept_admin", f.managerID, f.coID, f.deptID))
	...
	// ① 非司機身分(未建 logistics_drivers 列) → permission_denied。
	stranger := f.newLogisticsServer(t, logisticsIdentity("staff", f.managerID, f.coID, f.deptID))
	...
	// ② 司機一:只見自己那筆(共 1)。
	d1 := f.newLogisticsServer(t, logisticsIdentity("staff", f.driver1ID, f.coID, f.deptID))
	...
	drv := f.db.LogisticsDriver.Query().AllX(ctx)
	...
	if driverOfTarget == "" {
		t.Fatal("找不到司機一的 logistics_drivers 列")
	}
	if err := f.engine.DeleteTuple(ctx, driverOfTarget+"#assignee", "driver", "logistics_delivery:"+target); err != nil {
