// FleetService fleet 執行層首批(D32/10.1/10.4/10.12):建檔(司機/車輛)、
// 車次指派(route 粒度,version 樂觀鎖)、被指派司機的任務清單。
//
// 授權三層:rolePolicy fleet(middleware ability 閘門)→ 服務層 requireScope 與
// 部門範圍(deptScope/參照比對,RLS 兜底)→ OpenFGA instance 級(10.8:ListMyDeliveries
// 逐列 Check,被指派司機沿 driver#assignee 到達;tuple 經 AfterCommit 對帳,見 fleet_events.go)。
...
// FleetService 實作 salesorder.v1.FleetService。
type FleetService struct {
	db *ent.Client
	salesorderv1connect.UnimplementedFleetServiceHandler
}

// NewFleetService 建立 FleetService。
func NewFleetService(db *ent.Client) *FleetService {
	return &FleetService{db: db}
}

// RegisterFleetService 掛到 /api/v1(租戶 session + RLS,比照 SalesOrderService)。
func RegisterFleetService(mux *http.ServeMux, db *ent.Client) {
	path, handler := salesorderv1connect.NewFleetServiceHandler(NewFleetService(db),
		connect.WithInterceptors(requestid.Interceptor(), dbtenant.Interceptor(db)))
	mux.Handle(path, handler)
}

// fleetEngine 取每請求注入的 OpenFGA 引擎;缺席 = 接線失敗 → fail-closed(內部錯,
// 與 authorizeRPC「啟用卻無引擎」同語意)。刻意在交易內取:tuple 同步需要它,
// 提交前就失敗好過提交後靜默漏同步。
func fleetEngine(ctx context.Context) (*authzopenfga.Engine, error) {
...
func driverToProto(d *ent.FleetDriver) *salesorderv1.FleetDriver {
	out := &salesorderv1.FleetDriver{
...
func fleetDeliveryToProto(d *ent.FleetDelivery) *salesorderv1.FleetDelivery {
	out := &salesorderv1.FleetDelivery{
		Id:         strconv.Itoa(d.ID),
		CompanyId:  strconv.Itoa(d.CompanyID),
...
func (s *FleetService) CreateDriver(ctx context.Context, req *connect.Request[salesorderv1.CreateDriverRequest]) (*connect.Response[salesorderv1.CreateDriverResponse], error) {
	id, err := requireAuth(ctx)
	if err != nil {
		return nil, err
	}
	if err := requireScope(ctx, "fleet", "write"); err != nil {
		return nil, err
	}
	...
	exists, err := db.FleetDriver.Query().Where(
		fleetdriver.UserIDEQ(uid),
		fleetdriver.CompanyIDEQ(cid),
		fleetdriver.DeletedAtIsNil(),
	).Exist(ctx)
	if err != nil {
		return nil, toConnectError(err)
	}
	...
	if err := createAudit(ctx, tx, "fleet_driver", created.ID, cid, did, actor, map[string]any{
		"user_id": uid, "name": name,
	}); err != nil {
		return nil, err
	}
	if err := afterCommitFleetTuples(ctx, engine, driverObj(created.ID),
...
func (s *FleetService) CreateVehicle(ctx context.Context, req *connect.Request[salesorderv1.CreateVehicleRequest]) (*connect.Response[salesorderv1.CreateVehicleResponse], error) {
	id, err := requireAuth(ctx)
	if err != nil {
		return nil, err
	}
	...
	if err := afterCommitFleetTuples(ctx, engine, vehicleObj(created.ID),
...
func (s *FleetService) AssignDelivery(ctx context.Context, req *connect.Request[salesorderv1.AssignDeliveryRequest]) (*connect.Response[salesorderv1.AssignDeliveryResponse], error) {
	id, err := requireAuth(ctx)
	if err != nil {
		return nil, err
	}
	...
	if raw := strings.TrimSpace(req.Msg.GetDriverId()); raw != "" {
		...
		dq := db.FleetDriver.Query().Where(
			fleetdriver.ID(dvid), fleetdriver.CompanyIDEQ(cid), fleetdriver.DeletedAtIsNil())
		if did != nil {
			dq = dq.Where(fleetdriver.DepartmentIDEQ(*did))
		}
	...
	existing, err := db.FleetDelivery.Query().Where(
		fleetdelivery.RouteIDEQ(rid),
		fleetdelivery.CompanyIDEQ(cid),
		fleetdelivery.DeletedAtIsNil(),
	).First(ctx)
	if err != nil && !ent.IsNotFound(err) {
		return nil, toConnectError(err)
	}

	var saved *ent.FleetDelivery
	...
	default:
		...
		up := db.FleetDelivery.UpdateOneID(existing.ID).
			SetAssignedBy(actor).
	...
	if err := recordAuditBA(ctx, tx, "fleet_delivery", auditAction, saved.ID, cid, did, actor,
	...
	if err := afterCommitFleetTuples(ctx, engine, fleetObj(saved.ID),
	...
	return connect.NewResponse(&salesorderv1.AssignDeliveryResponse{
		Delivery: fleetDeliveryToProto(saved),
	}), nil
}

// ListMyDeliveries 我被指派的配送(10.12 閘門①:身分必須對應 fleet_drivers 列;
// 10.8 閘門②:逐列 OpenFGA Check —— tuple 缺席的列被過濾,不做 400/403 驚嚇)。
func (s *FleetService) ListMyDeliveries(ctx context.Context, req *connect.Request[salesorderv1.ListMyDeliveriesRequest]) (*connect.Response[salesorderv1.ListMyDeliveriesResponse], error) {
	id, err := requireAuth(ctx)
	if err != nil {
		return nil, err
	}
	if err := requireScope(ctx, "fleet", "read"); err != nil {
		return nil, err
	}
	cid, did, err := deptScope(id)
	if err != nil {
	...
	drvQ := db.FleetDriver.Query().Where(
		fleetdriver.UserIDEQ(me),
		fleetdriver.CompanyIDEQ(cid),
		fleetdriver.DeletedAtIsNil(),
	)
	if did != nil {
		drvQ = drvQ.Where(fleetdriver.DepartmentIDEQ(*did))
	}
	...
	rows, err := db.FleetDelivery.Query().Where(
		fleetdelivery.DriverIDEQ(drv.ID),
		fleetdelivery.DeletedAtIsNil(),
	).Order(ent.Desc(fleetdelivery.FieldID)).All(ctx)
	if err != nil {
		return nil, toConnectError(err)
	}
	// instance 級過濾(10.8):Check 不過的列視同不可見(與 list-objects 語意一致)。
	visible := make([]*ent.FleetDelivery, 0, len(rows))
	subject := "user:" + strconv.Itoa(me)
	for _, d := range rows {
		ok, cerr := engine.Check(ctx, subject, "can_read", fleetObj(d.ID))
		if cerr != nil {
			return nil, errcode.SysInternal.Error(nil)
		}
		if ok {
	...
	out := make([]*salesorderv1.FleetDelivery, 0, end-start)
	for _, d := range visible[start:end] {
		out = append(out, fleetDeliveryToProto(d))
	}
