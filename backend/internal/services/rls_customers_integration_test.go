...
// 驗證客戶域的服務路徑（27 處 dbtenant.Client、11 處請求交易）在 RLS 生效時確實在租戶範圍內
// 讀寫 —— 漏掛 dbtenant.Client 的查詢在 app_rw 上會 fail-closed（0 列），故本測試是這些
// 呼叫點唯一的守門。
//
...
func TestIntegrationCustomerServiceUnderAppRole(t *testing.T) {
	...
	actor := insertRLSUser(t, admin, coA, "svc-admin@example.com")
	...
	RegisterCustomerServices(mux, client, "http://localhost:3000") // 內含 dbtenant.HandlerOption(請求交易)
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// middleware 在生產做的事(server.go 的 identityFor → WithRLS):把身分導出的 RLS scope
		// 注入 ctx,請求交易開啟時由 driver 裝飾器 SET LOCAL。
		ctx = auth.WithRLS(ctx, auth.RLSScope{
			UserID: itoa(actor), CompanyID: itoa(coA), DataScope: auth.DataScopeCompany, CompanyActive: true,
		})
	...
	// 讀:租戶範圍生效 → 只看得到自己公司的客戶(漏掛 dbtenant.Client → 連自己的都看不到)。
	res, err := svc.ListCustomers(ctx, connect.NewRequest(&customersv1.ListCustomersRequest{Page: 1, PageSize: 50}))
	if err != nil {
		t.Fatalf("ListCustomers: %v", err)
	}
	got := res.Msg.GetCustomers()
	if len(got) != 1 || got[0].GetId() != itoa(custA) {
		t.Fatalf("應只看到公司 %d 的客戶 %d,得到 %d 筆(%v)", coA, custA, len(got), got)
	}

	// 跨租戶讀:B 的客戶在 A 的請求交易裡「不存在」(RLS 過濾,而非權限錯誤)。
	...
	// 寫:新增地址(請求交易內 + WITH CHECK + 稽核同交易)→ 成功且提交後落地。
	if _, err := svc.AddAddress(ctx, connect.NewRequest(&customersv1.AddAddressRequest{
		CustomerId: itoa(custA), Type: "shipping", RecipientName: "收件人", AddressLine: "台北市",
	})); err != nil {
		t.Fatalf("AddAddress(app_rw + 租戶範圍): %v", err)
	}
	var n int
	if err := admin.QueryRow(`SELECT count(*) FROM customer_addresses WHERE customer_id = $1 AND company_id = $2`,
		custA, coA).Scan(&n); err != nil {
		t.Fatalf("查地址列: %v", err)
	}
	if n != 1 {
...
func insertRLSUser(t *testing.T, db *sql.DB, companyID int, email string) int {
	t.Helper()
	var id int
	if err := db.QueryRow(
		`INSERT INTO users (email, name, role, password_hash, company_users)
		 VALUES ($1, '公司管理員', 'company_admin', 'x', $2) RETURNING id`, email, companyID).Scan(&id); err != nil {
