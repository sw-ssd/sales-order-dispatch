...
func (_q *CompanyQuery) loadDepartments(ctx context.Context, query *DepartmentQuery, nodes []*Company, init func(*Company), assign func(*Company, *Department)) error {
	...
	for _, n := range neighbors {
		fk := n.company_departments
		if fk == nil {
			return fmt.Errorf(`foreign-key "company_departments" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "company_departments" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
