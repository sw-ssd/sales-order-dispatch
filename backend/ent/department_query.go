...
func (_q *DepartmentQuery) QueryCompany() *CompanyQuery {
...
func (_q *DepartmentQuery) WithCompany(opts ...func(*CompanyQuery)) *DepartmentQuery {
	query := (&CompanyClient{config: _q.config}).Query()
...
func (_q *DepartmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Department, error) {
	...
	if query := _q.withCompany; query != nil {
		if err := _q.loadCompany(ctx, query, nodes, nil,
			func(n *Department, e *Company) { n.Edges.Company = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *DepartmentQuery) loadCompany(ctx context.Context, query *CompanyQuery, nodes []*Department, init func(*Department), assign func(*Department, *Company)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Department)
	for i := range nodes {
		if nodes[i].company_departments == nil {
			continue
		}
		fk := *nodes[i].company_departments
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "company_departments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *DepartmentQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Department, init func(*Department), assign func(*Department, *User)) error {
	...
	for _, n := range neighbors {
		...
		if fk == nil {
			return fmt.Errorf(`foreign-key "department_users" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "department_users" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
