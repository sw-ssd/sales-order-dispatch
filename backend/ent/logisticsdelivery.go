...
// LogisticsDelivery is the model entity for the LogisticsDelivery schema.
...
func (*LogisticsDelivery) scanValues(columns []string) ([]any, error) {
	values := make([]any, len(columns))
	for i := range columns {
		switch columns[i] {
		case logisticsdelivery.FieldID, logisticsdelivery.FieldCompanyID, logisticsdelivery.FieldDepartmentID, logisticsdelivery.FieldRouteID, logisticsdelivery.FieldDriverID, logisticsdelivery.FieldVehicleID, logisticsdelivery.FieldAssignedBy, logisticsdelivery.FieldVersion:
...
// to the LogisticsDelivery fields.
func (_m *LogisticsDelivery) assignValues(columns []string, values []any) error {
	if m, n := len(values), len(columns); m < n {
		return fmt.Errorf("mismatch number of scan values: %d != %d", m, n)
	}
	for i := range columns {
		switch columns[i] {
		case logisticsdelivery.FieldID:
...
// Value returns the ent.Value that was dynamically selected and assigned to the LogisticsDelivery.
// This includes values selected through modifiers, order, etc.
func (_m *LogisticsDelivery) Value(name string) (ent.Value, error) {
	return _m.selectValues.Get(name)
}

// Update returns a builder for updating this LogisticsDelivery.
// Note that you need to call LogisticsDelivery.Unwrap() before calling this method if this LogisticsDelivery
// was returned from a transaction, and the transaction was committed or rolled back.
func (_m *LogisticsDelivery) Update() *LogisticsDeliveryUpdateOne {
	return NewLogisticsDeliveryClient(_m.config).UpdateOne(_m)
}

// Unwrap unwraps the LogisticsDelivery entity that was returned from a transaction after it was closed,
// so that all future queries will be executed through the driver which created the transaction.
func (_m *LogisticsDelivery) Unwrap() *LogisticsDelivery {
	_tx, ok := _m.config.driver.(*txDriver)
	if !ok {
		panic("ent: LogisticsDelivery is not a transactional entity")
	}
...
func (_m *LogisticsDelivery) String() string {
	var builder strings.Builder
	builder.WriteString("LogisticsDelivery(")
...
// LogisticsDeliveries is a parsable slice of LogisticsDelivery.
type LogisticsDeliveries []*LogisticsDelivery
